...
might arise when working on code collabotatively.
...
::: {.notes}
Not that we have setup and activated a Virtual Environment and cloned the TopoStats repository its time to install it.

Start a terminal (or Conda Powershell) and navigate to the location you installed TopoStats using ~cd~.

Check you have activated your Conda environment if not
:::
...
Now install TopoStats with the command `pip install -e .` and it should start downloading a bunch of files that
TopoStats depends on then install all of them.
:::
...
within your current working directory. If you use the `-q` or `--quiet` flag it will be considerably less verbose, but
it can often be useful to be verbose, particularly when submitting issues which we will come to in a bit.
...
::: {.notes}
We now return to Git because in order to test new features you need to be able to switch between what is known as
branches.
...
You can use GitKraken to switch branches, switch to `topostats-git`.
...
::: {.notes}
Switch branches using Git Kraken.
...
You should have seen that the last message was
